...
class PDF_MC_Table extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->addSociete(utf8_decode($this->fde_nombre), utf8_decode($direccion), utf8_decode($this->fde_email), utf8_decode($this->fde_web), utf8_decode($this->fde_web));
...
if ($this->impresion['f_detallada_QRCODE']) {
            $cadena = 'BEGIN:VCARD' . "\n";
            $cadena .= 'VERSION:2.1' . "\n";
            $cadena .= 'ADR;WORK:;;' . $this->fdf_tipodocumento . ';' . $this->fdf_codigo . "\n"; 
           $cadena .= 'END:VCARD';

            $qrcode1 = new QRcode($cadena, 'L');
            $qrcode1->disableBorder();
            $background = array(255, 255, 255);
            $color = array(0, 0, 0);
            $qrcode1->displayFPDF($this, 150,40, 20, $background, $color);
            unset($qrcode1);
            $this->SetColorRelleno($this->color_relleno);
        }
