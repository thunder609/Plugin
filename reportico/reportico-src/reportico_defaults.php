    ...
    */
    function reportico_defaults($reportico)
    {
            ...
            if ( $reportico->pdf_engine == "tcpdf" )
            {
                ...
                $reportico->create_page_header("H1", 1, "{REPORT_TITLE}{STYLE border-width: 1 0 1 0; margin: 15px 0px 0px 0px; border-color: #000000; font-size: 18; border-style: solid;padding:5px 0px 5px 0px; height:1cm; background-color: #ffffff; color: #000000; text-align: center}" );
                ...
                */
                ...
                // Create Image on every page of PDF
                $reportico->create_page_header("H3", 1, "Hora: date('Y-m-d H:i:s'){STYLE font-size: 10; text-align: right; font-style: italic;}" );
                $reportico->set_page_header_attribute("H3", "ShowInHTML", "no" );
                $reportico->set_page_header_attribute("H3", "ShowInPDF", "no" );
                $reportico->set_page_header_attribute("H3", "justify", "right" );

                // Create Page No on bottom of PDF page
                $reportico->create_page_footer("F1", 2, "Página: {PAGE}{STYLE border-width: 0 0 0 0; margin: 25 0 0 0; font-style: italic; }" );
            }
            else   // FPDF page headers
            {
                // Create Report Title Page Header on every page of PDF
                $reportico->create_page_header("H1", 2, "{REPORT_TITLE}{STYLE border-width: 1 0 1 0; margin: 15px 0px 0px 0px; border-color: #000000; font-size: 18; border-style: solid;padding:5px 0px 5px 0px; height:1cm; background-color: #ffffff; color: #000000}" );
                $reportico->set_page_header_attribute("H1", "ShowInHTML", "no" );
